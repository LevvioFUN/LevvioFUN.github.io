...
landingApp
    ...
    .controller('landingCtrl', ['$scope', '$location', '$http', '$q', '$sce', 'documentLocationParams', '$window', '$document', function controller($scope, $location, $http, $q, $sce, documentLocationParams, $window, $document) {
        ...
        $scope.redirect = function (url, urlToRedirect) {
            if (vectrk) {
                var redirect = vectrk('augmentUrl', $scope.toAbsoluteUrl(urlToRedirect));
                if ($scope.demo) $scope.demoRedirect.push({name: url}); //performance detect 
                url = url.replace('[RANDOM]', new Date().getTime());
                url = url.replace('[URLREDIRECT]', encodeURIComponent(redirect));
                var data = vectrk('url', 'adv_ban_cli', { v_n: 'click', v_rur: url }, true);
                window.open(data);
            }
        };

        $scope.redirectLanding = function (url) {
            if (vectrk) {
                var redirect = vectrk('augmentUrl', $scope.toAbsoluteUrl(url));
                var data = vectrk('url', 'adv_ban_cli', { v_n: 'click', v_rur: redirect }, true);
                window.open(data);
            }
        };

        $scope.redirectToSite= function (url) {
            $scope.addPixel(url);
            url = url.replace('[RANDOM]', new Date().getTime());
            window.open(url);
        };
        ...
        $scope.addPixel = function (src) {
            new Image().src = src.replace('[RANDOM]', new Date().getTime());
        }
        ...
        $scope.locatorSelectedStoreUrl = function () {
            if ($scope.locatorSelectedStore) {
                ...
                params.id = $scope.locatorSelectedStore.code_mag;
    ...
    .directive('tagPlan', ['documentLocationParams', function (documentLocationParams) {
        return {
            link: function link(scope, element, attrs) {
                if (!window.vectrk) {
                    return;
